    ...
    public class Player : NetworkBehaviour
    {
        ...
        private void Start()
        {
            if (GlobalReferences.instance != null)
                GlobalReferences.instance._localPlayer = gameObject;
        }
        ...
        /// </remarks>
        [ServerRpc]
        public void RpcRandomTeleportServerRpc()
        {
            var oldPosition = transform.position;
            transform.position = GetRandomPositionOnXYPlane();
            var newPosition = transform.position;
            print($"{nameof(RpcRandomTeleportServerRpc)}() -> {nameof(OwnerClientId)}: {OwnerClientId} --- {nameof(oldPosition)}: {oldPosition} --- {nameof(newPosition)}: {newPosition}");
        }
